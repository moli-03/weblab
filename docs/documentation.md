...
sequenceDiagram
    ...
    U->>W: Login-Formular ausfüllen
    W->>A: POST /api/auth/login
    A->>D: Benutzer abfragen
    D-->>A: Benutzer-Daten
    A->>A: bcrypt verify
    alt Passwort korrekt
        A->>A: JWT Token erstellen
        A->>D: Login-Audit protokollieren
        A-->>W: JWT Token + User
        W->>W: Token im localStorage speichern
        W-->>U: Redirect zu Dashboard
    else Passwort falsch
        A->>D: Login-Fehlschlag protokollieren
        A-->>W: 401 Unauthorized
        W-->>U: Fehlermeldung anzeigen
    end
```

**Besonderheiten:**
- Passwort-Hashing mit bcrypt für sichere Authentifizierung
- Audit-Log für alle Login-Versuche (erfolgreich und fehlgeschlagen)
- JWT Token für stateless Authentication
- Client-side Token-Speicherung für nachfolgende Requests

## Workspace-Einladung

```mermaid
sequenceDiagram
    participant C as CTO 
    participant W as Web App
    participant A as API Server
    participant D as Database
    participant I as Invited User

    C->>W: Einladung erstellen
    W->>A: POST /api/workspaces/{id}/invite
    A->>A: Berechtigung prüfen
    A->>A: Einladungs-Token generieren
    A->>D: Invite Record erstellen
    A-->>W: Invite-Link
    W-->>C: Share-Dialog mit Link

    C->>I: Invite-Link teilen
    
    I->>W: Invite-Link öffnen
    W->>A: GET /api/invite/{token}
    A->>D: Token validieren & Invite laden
    
    alt Token gültig & nicht abgelaufen
        D-->>A: Valid Invite + Workspace Info
        A-->>W: Workspace Details
        W-->>I: Einladung anzeigen
        
        I->>W: Einladung akzeptieren
        W->>A: POST /api/invite/{token}
        A->>D: Workspace-Mitgliedschaft erstellen
        A->>D: Invite als "used" markieren
        A-->>W: 200 + Redirect-URL
        W-->>I: Redirect zu Workspace
    else Token ungültig
        A-->>W: 404 Not Found
        W-->>I: "Einladung ungültig" Fehler
    end
```

**Besonderheiten:**
- Einmalige Verwendung von Einladungs-Tokens
- Ablaufzeit für Sicherheit
- Automatische Weiterleitung nach erfolgreicher Annahme
